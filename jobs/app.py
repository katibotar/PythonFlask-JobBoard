...
def job(job_id):
    job = execute_sql("SELECT job.id, job.title, job.description, job.salary, employer.id "
                      "as employer_id, employer.name as employer_name FROM job JOIN "
                      "employer ON employer.id = job.employer_id WHERE job.id = ?", [job_id], single=True)
...
def employer(employer_id):
    ...
    reviews = execute_sql("SELECT review, rating, title, date, status FROM review JOIN "
                       "employer ON employer.id = review.employer_id WHERE employer.id = ?", [employer_id])
